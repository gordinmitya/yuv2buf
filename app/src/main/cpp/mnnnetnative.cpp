...
void convert(JNIEnv *env, jbyte *bufferData, jint jwidth, jint jheight,
             jlong tensorPtr, jint srcType,
             jint destFormat, jint filterType, jint wrap, jfloatArray matrixValue_,
             jfloatArray mean_,
             jfloatArray normal_) {
...
Java_com_taobao_android_mnn_MNNNetNative_nativeConvertBufferToTensor(
        JNIEnv *env, jclass type, jobject jbuffer, jint jwidth, jint jheight,
        jlong tensorPtr, jint srcType,
        jint destFormat, jint filterType, jint wrap, jfloatArray matrixValue_, jfloatArray mean_,
        jfloatArray normal_) {
    jbyte *bufferData = static_cast<jbyte *>(env->GetDirectBufferAddress(jbuffer));
    convert(env, bufferData, jwidth, jheight, tensorPtr, srcType, destFormat, filterType, wrap,
            matrixValue_, mean_, normal_);
    return JNI_TRUE;
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_taobao_android_mnn_MNNNetNative_nativeConvertArrayToTensor(
